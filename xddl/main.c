...
int main(const int argc, const char *const *const argv)
{
	...
	{
		fprintf(stderr, "\033[31msetenv failed, errno: %s\033[0m\n", strerror(errno));
		return -1;
	}
	...
	if (pid == -1)
	{
		fprintf(stderr, "\033[31mfork failed, errno: %s\033[0m\n", strerror(errno));
		return -1;
	}
	else if (pid == 0)
	{
		...
		{
			fprintf(stderr, "\033[31mfreopen failed, errno: %s\033[0m\n", strerror(errno));
			exit(1);
		}
		if (execl(argv[1], argv[1], nullptr) == -1)
		{
			fclose(stdout);
			fprintf(stderr, "\033[31mexecl failed, errno: %s\033[0m\n", strerror(errno));
			exit(1);
		}
		exit(1);// на всякий
	}
	...
	if (WEXITSTATUS(rv) != 0)
	{
		fprintf(stderr, "\033[31mWEXITSTATUS: %d\033[0m\n", WEXITSTATUS(rv));
		exit(1);
	}
	...
	{
		fprintf(stderr, "\033[31mfopen failed, errno: %s\033[0m\n", strerror(errno));
		exit(1);
	}
	...
	{
		fprintf(stderr, "\033[31mremove failed, errno: %s\033[0m\n", strerror(errno));
	}
